...
class PostgreSQLDatabase(CommonDatabase):
    ...
    CREATE_SONGS_TABLE = f"""
        ...
        ,   "{FIELD_SONGNAME}" VARCHAR(500) NOT NULL
    ...
    CREATE_CHECKSONG_TABLE = f"""
       CREATE TABLE IF NOT EXISTS "{CHECK_SONG_TABLENAME}" (
            "id" serial NOT NULL
        ,   "check_song_id" INT NOT NULL
        ,   "check_song_name" VARCHAR(500)
        ,   "original_song_id" INT NOT NULL
        ,   "original_song_name" VARCHAR(500)

        ,   "input_total_hashes" INT NOT NULL
        ,   "fingerprinted_hashes_in_db" INT NOT NULL
        ,   "time_similar" float(2)
        ,   "accuracy_similar" float(2)
        ,   "fingerprint_match" int
        ,   "hashes_matched_in_input" INT NOT NULL
        ,   "input_confidence" float(2)
        ,   "fingerprinted_confidence" float(2)
        ,   "start_match_target" INT NOT NULL
        ,   "start_match_source" INT NOT NULL
        ,   CONSTRAINT "pk_{CHECK_SONG_TABLENAME}_id" PRIMARY KEY ("id")
        );
    """
    ...
    INSERT_CHECK_SONG = f"""
        INSERT INTO "{CHECK_SONG_TABLENAME}" (
            "check_song_id"
        ,   "check_song_name"
        ,   "original_song_id"
        ,   "original_song_name"
    ...
    INSERT_SONG = f"""
        INSERT INTO "{SONGS_TABLENAME}" ("{FIELD_SONG_ID}","{FIELD_SONGNAME}", "{FIELD_FILE_SHA1}","{FIELD_TOTAL_HASHES}")
    ...
    def insert_song(self,song_id: int, song_name: str, file_hash: str, total_hashes: int) -> int:
        """
