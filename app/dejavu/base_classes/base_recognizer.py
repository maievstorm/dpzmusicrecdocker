...
class BaseRecognizer(object, metaclass=abc.ABCMeta):
    ...
    def _recognize(self, data,id_check,name_check,is_save) -> Tuple[List[Dict[str, any]], int, int, int]:
        ...
        t = time()
        final_results = self.dejavu.align_matches(matches, dedup_hashes, len(hashes),id_check,name_check,is_save)
        align_time = time() - t
